  ...
  let str_of_type loc type_decl =
    ...
    let typ_of =
      ...
      | Ptype_abstract, Some manifest ->
        [value_binding ~pat:(pvar typ_of_lid) ~expr:(polymorphize (expr_of_typ ~loc manifest))]
      | Ptype_record labels, _ ->
        ...
        let analyse_field =
          fun label_declaration ->
            ...
            let doc = Common.get_doc ~loc (Attribute.get Attrs.label_doc label_declaration) pld_attributes in
            let fget = [%expr fun _r -> [%e pexp_field (evar "_r") (Located.mk (Lident f_name))]] in
            ...
            let expr = pexp_record
                (ListLabels.map ~f:(fun (x,y) -> Located.mk (Ldot (Ldot (Lident "Rpc", "Types"), x)), y)
                       ...
                       [%expr
                         Rpc.Types.Struct
                           ( { Rpc.Types.fields = [%e boxed_fields]
                             ...
                             ; Rpc.Types.version = [%e expr_of_option ~loc (Attribute.get Attrs.td_version type_decl) ]
      ...
      | Ptype_variant constrs, _ ->
        ...
        let analyse_case constructor =
          ...
          let typs =
            ...
            | Pcstr_record _ ->
              Location.raise_errorf "%s: record variants are not supported"
                deriver
          in
          let contents =
            match typs with
            ...
            | [t1; t2; t3; t4] ->
              [%expr
                Tuple4
                  ...
                  , [%e expr_of_typ ~loc t3]
                  , [%e expr_of_typ ~loc t4] )]
            | _ -> Location.raise_errorf "%s: Tuples with arity > 4 are not supported" deriver
          in
          ...
          let vpreview =
            pexp_function
              ( [ case ~lhs:(ppat_construct (Located.mk (lident cname)) pat)
                    ~guard:None ~rhs:[%expr Some [%e expr ]] ]
                @ vpreview_default )
          in
          let vreview =
            pexp_function
    ...
    in
    let doc = get_doc ~loc (Attribute.get Attrs.td_doc type_decl) type_decl.ptype_attributes in
    let name = type_decl.ptype_name.txt in
    typ_of @ [
...
let my_str_type_decl ~loc ~path:_ (rec_flag, tds) =
  pstr_value_list ~loc rec_flag (List.concat (ListLabels.map ~f:(Typ_of.str_of_type loc) tds))

let str_type_decl =
  Deriving.Generator.make_noarg my_str_type_decl

let deriver =
  Deriving.add deriver ~str_type_decl
