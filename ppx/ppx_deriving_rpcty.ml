...
let attr_version attrs =
  Ppx_deriving.attr ~deriver "version" attrs
  |> Ppx_deriving.Arg.(get_attr ~deriver expr)
...
module Typ_of = struct
  ...
  let rec expr_of_typ typ =
    let expr =
      ...
      | [%type : [%t ? typ] option] -> [%expr Option [%e expr_of_typ typ]]
      | {ptyp_desc= Ptyp_constr ({txt= lid; _}, _); _} ->
          [%expr
            [%e
              Exp.ident
                (mknoloc (Ppx_deriving.mangle_lid (`Prefix "typ_of") lid))]]
  ...
  let str_of_type type_decl =
    ...
    let typ_of_lid =
      Ppx_deriving.mangle_type_decl (`Prefix "typ_of") type_decl
    in
    let typ_of =
      ...
      | Ptype_record labels, _ ->
          let fields =
            ...
            labels
            |> List.map
                 (fun {pld_name= {txt= fname; _}; pld_type; pld_attributes; _}
                 ->
                   ...
                   let fget =
                     [%expr
                       fun _r ->
                         [%e Exp.field (evar "_r") (mknoloc (Lident fname))]]
                   in
                   let fset =
                     [%expr
                       fun v _s ->
                         [%e
                           record
                             [(fname, [%expr v])]
                             ?over:(if one_field then None else Some [%expr _s])]]
                   in
                   ( fname
                   , rpc_name
                   , field_name
                   ...
                   , [%expr
                       ...
                       [%e
                         record
                           ...
                           ; ( "fversion"
                             , match attr_version pld_attributes with
                               | Some v -> [%expr Some [%e v]]
                               | None -> [%expr None] )
                           ; ("fget", fget)
                           ; ("fset", fset) ]]]
                   , default ) )
          ...
          let field_name_bindings =
            List.map
              (fun (_, _, field_name, _, record, _) ->
                Vb.mk
                  (Pat.constraint_ (pvar field_name)
          ...
          let boxed_fields =
            list
              (List.map
                 (fun (_, _, field_name, _, _, _) ->
                   [%expr BoxedField [%e Exp.ident (lid field_name)]] )
                 fields)
          in
          let construct_record =
            List.fold_left
              (fun expr (_, rpc_name, field_name, pld_type, _, def) ->
                match def with
                | Some d ->
                    [%expr
                      ( match
                          getter.Rpc.Types.field_get [%e str rpc_name]
                            [%e expr_of_typ pld_type]
                        with
                      | Result.Ok _ as y -> y
                      | Result.Error _ -> Result.Ok [%e d] )
                      >>= fun [%p pvar field_name] -> [%e expr]]
                | None ->
                    [%expr
                      getter.Rpc.Types.field_get [%e str rpc_name]
                        [%e expr_of_typ pld_type]
              ...
              [%expr
                return
                  [%e
                    Exp.record
                      (List.map
                         (fun (fname, _, field_name, _, _, _) ->
                           (mknoloc (Lident fname), evar field_name) )
                         fields)
                      None]]
              fields
          in
                      ...
                      [%expr
                        Struct
                          ( { fields= [%e boxed_fields]
                            ...
                            ; version=
                                ( match%e
                                    attr_version type_decl.ptype_attributes
                                  with
                                | Some v -> [%expr Some [%e v]]
                                | None -> [%expr None] )
                            ; constructor=
            ...
            |> List.map
                 ...
                 ->
                   ...
                   let typs =
                     ...
                     | Pcstr_record _ ->
                         raise_errorf "%s: record variants are not supported"
                           deriver
                   in
                   let contents =
                     match typs with
                     ...
                     | [t1; t2] ->
                         [%expr
                           Tuple ([%e expr_of_typ t1], [%e expr_of_typ t2])]
                     | [t1; t2; t3] ->
                         [%expr
                           Tuple3
                             ( [%e expr_of_typ t1]
                   ...
                   let vpreview_default =
                     ...
                     else [Exp.case (Pat.any ()) [%expr None]]
                   in
                   let vpreview =
                     Exp.function_
                       ( [ Exp.case (pconstr name pattern)
                             [%expr Some [%e tuple args]] ]
                       @ vpreview_default )
                   in
                   let vreview =
                     Exp.function_
                       [Exp.case (ptuple pattern) (constr name args)]
                   in
                   let variant =
                     [%expr
                       BoxedTag
                         [%e
                           record
                             ...
                             ; ("tcontents", contents)
                             ; ( "tversion"
                               , match attr_version pcd_attributes with
                                 | Some v -> [%expr Some [%e v]]
                                 | None -> [%expr None] )
                   ...
                   let vconstructor_case =
                     Exp.case
                       (Pat.constant (Pconst_string (lower_rpc_name, None)))
                       [%expr
                         Rresult.R.bind
                           (t.tget [%e contents])
                           [%e
                             Exp.function_
                               [ Exp.case (ptuple pattern)
                                   [%expr Rresult.R.ok [%e constr name args]]
                               ]]]
                   in
          ...
          let vconstructor =
            [%expr
              fun s' t ->
                ...
                [%e Exp.match_ (evar "s") (List.map snd cases @ default)]]
          in
          [ Vb.mk (pvar typ_of_lid)
              (wrap_runtime
                 (polymorphize
                ...
                (record
                   ...
                   ; ( "ty"
                     , Ppx_deriving.poly_apply_of_type_decl type_decl
                         (Exp.ident (lid typ_of_lid)) ) ]))) ]
end

let rpcty_strs_of_type type_decl = Typ_of.str_of_type type_decl
  ...
  register
    (create deriver
       ~type_decl_str:(fun ~options:_ ~path:_ type_decls ->
         [ Str.value Recursive
             (List.concat (List.map rpcty_strs_of_type type_decls)) ] )
       ())
