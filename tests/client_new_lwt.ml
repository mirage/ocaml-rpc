...
} [@@deriving rpcty] (* Generates return_record_def *)

type variant_t =
  | Foo of string list
  | Bar
  | Baz of float
[@@deriving rpcty] (* Generates variant_t_def *)
...
(* Server-side implementations of these RPCs *)
module ImplM = struct
  open Rpclwt.M
...
module Server = API(Rpclwt.GenServer)

let server_funcs =
  Rpclwt.GenServer.empty ()
  |> Server.rpc1 ImplM.rpc1
  |> Server.rpc2 ImplM.rpc2
  |> Server.rpc3 ImplM.rpc3

(* Generate Client functions *)
module Client = API(Rpclwt.GenClient)

let main () =
  (* A handy monad for dealing with these RPC functions. If we didn't have to do the GADT-injectivity thing of
     wrapping Lwt.t in a record, this would just be the composition of the Lwt and Result monads. *)
  let open Rpclwt.M in

  (* Wrap an RPC function with debug prints. We'll round-trip through the Jsonrpc module for good measure. *)
  let wrap_rpc rpc_fn call =
    let open Lwt in
  ...
  let rpc = rpc (Rpclwt.GenServer.server funcs) in
