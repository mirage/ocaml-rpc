...
let test_int () = check_marshal_unmarshal (1, Rpc.Int 1L, rpc_of_test_int, test_int_of_rpc)
...
let test_bad_int_string () = check_unmarshal_error test_int_of_rpc (Rpc.String "tree")
...
let test_bad_int32 () = check_unmarshal_error test_int32_of_rpc (Rpc.Float 1.0)
let test_bad_int32_string () = check_unmarshal_error test_int32_of_rpc (Rpc.String "moo")
...
let test_bad_int64 () = check_unmarshal_error test_int64_of_rpc (Rpc.Float 1.0)
...
let test_string () =
  check_marshal_unmarshal
    ("test string", Rpc.String "test string", rpc_of_test_string, test_string_of_rpc)


let test_bad_string () = check_unmarshal_error test_string_of_rpc (Rpc.Int 1L)

type test_float = float [@@deriving rpc]

let check_unmarshal_float_ok = check_unmarshal_ok Testable.float test_float_of_rpc

let test_float () =
  check_marshal_unmarshal (2.0, Rpc.Float 2.0, rpc_of_test_float, test_float_of_rpc)


let test_float_from_int () = check_unmarshal_float_ok 1.0 (Rpc.Int 1L)
let test_float_from_int32 () = check_unmarshal_float_ok 1.0 (Rpc.Int32 1l)
let test_float_from_string () = check_unmarshal_float_ok 1.0 (Rpc.String "1.0")
let test_bad_float () = check_unmarshal_error test_float_of_rpc (Rpc.Enum [])
let test_bad_float_string () = check_unmarshal_error test_float_of_rpc (Rpc.String "xxy")
...
let test_bad_bool () = check_unmarshal_error test_bool_of_rpc (Rpc.String "true")

type test_char = char [@@deriving rpc]

let test_char () =
  check_marshal_unmarshal
    ('x', Rpc.Int (Char.code 'x' |> Int64.of_int), rpc_of_test_char, test_char_of_rpc)


let test_bad_char () = check_unmarshal_error test_char_of_rpc (Rpc.String "x")
...
let test_int_list () =
  check_marshal_unmarshal
    ...
    , rpc_of_test_int_list
    , test_int_list_of_rpc )
...
let test_dict () =
  check_marshal_unmarshal
    ( [ "foo", 1; "bar", 2; "baz", 3 ]
    , Rpc.Dict [ "foo", Rpc.Int 1L; "bar", Rpc.Int 2L; "baz", Rpc.Int 3L ]
    , rpc_of_test_dict
    , test_dict_of_rpc )
...
let test_dict_key () =
  check_marshal_unmarshal
    ( [ "foo", 1; "bar", 2; "baz", 3 ]
    , Rpc.Dict [ "foo", Rpc.Int 1L; "bar", Rpc.Int 2L; "baz", Rpc.Int 3L ]
    , rpc_of_test_dict_key
    , test_dict_key_of_rpc )
...
let test_int_array () =
  check_marshal_unmarshal
    ( [| 1; 2; 3; 4 |]
    , Rpc.Enum [ Rpc.Int 1L; Rpc.Int 2L; Rpc.Int 3L; Rpc.Int 4L ]
    , rpc_of_test_int_array
    , test_int_array_of_rpc )
...
let test_tuple3 () =
  check_marshal_unmarshal
    ( (3, "hi", 'c')
    , Rpc.Enum [ Rpc.Int 3L; Rpc.String "hi"; Rpc.Int (Char.code 'c' |> Int64.of_int) ]
    , rpc_of_test_tuple3
    , test_tuple3_of_rpc )
...
let test_option () =
  check_marshal_unmarshal
    (Some 1, Rpc.Enum [ Rpc.Int 1L ], rpc_of_test_option, test_option_of_rpc)
...
let test_bad_option () = check_unmarshal_error test_option_of_rpc (Rpc.Int 5L)
...
let test_variant1 () =
  check_marshal_unmarshal
    ( VOne 1
    , Rpc.Enum [ Rpc.String "VOne"; Rpc.Int 1L ]
    , rpc_of_test_variant
    , test_variant_of_rpc )
...
let test_variant_case () =
  check_unmarshal_ok
    (Testable.from_rpc_of_t rpc_of_test_variant)
    test_variant_of_rpc
    VNone
    (Rpc.String "vnone")
...
let test_variant_name () =
  check_marshal_unmarshal
    ( VThree 5
    , Rpc.Enum [ Rpc.String "bob"; Rpc.Int 5L ]
    , rpc_of_test_variant_name
    , test_variant_name_of_rpc )


let test_variant_name2 () =
  check_marshal_unmarshal
    (VFour, Rpc.String "lofty", rpc_of_test_variant_name, test_variant_name_of_rpc)
...
let test_record_case () =
  check_unmarshal_ok
    (Testable.from_rpc_of_t rpc_of_test_record)
    test_record_of_rpc
    { fiEld1 = 7; fiEld2 = "banana" }
    (Rpc.Dict [ "field1", Rpc.Int 7L; "FIELD2", Rpc.String "banana" ])
...
type test_record_attrs = { field5 : int [@key "foo"] } [@@deriving rpc]

let test_record_attrs () =
  check_marshal_unmarshal
    ( { field5 = 6 }
    , Rpc.Dict [ "foo", Rpc.Int 6L ]
    , rpc_of_test_record_attrs
    , test_record_attrs_of_rpc )
...
let test_poly () =
  ...
  check_marshal_unmarshal
    ( x
    , Rpc.Enum [ Rpc.Int 1L; Rpc.Int 2L; Rpc.Int 3L ]
    , rpc_of_test_poly Rpc.rpc_of_int
    , test_poly_of_rpc Rpc.int_of_rpc )
...
let test_polyvar2 () =
  check_marshal_unmarshal
    ( `two 2
    , Rpc.Enum [ Rpc.String "two"; Rpc.Int 2L ]
    , rpc_of_test_polyvar
    , test_polyvar_of_rpc )
...
let test_polyvar_case () =
  check_unmarshal_ok
    (Testable.from_rpc_of_t rpc_of_test_polyvar)
    test_polyvar_of_rpc
    (`thRee (4, 5))
    (Rpc.Enum [ Rpc.String "THREE"; Rpc.Enum [ Rpc.Int 4L; Rpc.Int 5L ] ])


type test_pvar_inherit =
  [ `four of string
  | test_polyvar
  ]
[@@deriving rpc]
...
let test_pvar_inherit () =
  check_marshal_unmarshal
    (`one, Rpc.String "one", rpc_of_test_pvar_inherit, test_pvar_inherit_of_rpc)


let test_pvar_inherit2 () =
  check_marshal_unmarshal
    ( `four "apple"
    , Rpc.Enum [ Rpc.String "four"; Rpc.String "apple" ]
    , rpc_of_test_pvar_inherit
...
let test_default_enum () =
  check_unmarshal_ok
    (Testable.from_rpc_of_t rpc_of_enum)
    enum_of_rpc
    `default
    (Rpc.String "unknown_enum");
  check_unmarshal_ok
    (Testable.from_rpc_of_t rpc_of_enum)
    enum_of_rpc
    `default
...
let test_enum_string_map () =
  check_marshal_unmarshal
    ( [ `x, "x"; `y, "y"; `z, "z" ]
    , Rpc.Dict [ "x", Rpc.String "x"; "y", Rpc.String "y"; "z", Rpc.String "z" ]
    , rpc_of_enum_string_map
    , enum_string_map_of_rpc )
...
let test_enum_string_map2 () =
  check_marshal_unmarshal
    ( [ `a, "x"; `b, "y"; `c, "z" ]
    , Rpc.Dict [ "a", Rpc.String "x"; "b", Rpc.String "y"; "c", Rpc.String "z" ]
    , rpc_of_enum_string_map2
    , enum_string_map2_of_rpc )
