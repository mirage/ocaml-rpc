...
let check_marshal_unmarshal :
    'a * Rpc.t * ('a -> Rpc.t) * (Rpc.t -> 'a) -> unit =
 fun (x, r, marshal, unmarshal) ->
  let r' = marshal x in
  let x' = unmarshal r in
  Alcotest.check
    (Testable.from_rpc_of_t marshal)
    "same after marshal->unmarshal" x x' ;
...
let check_unmarshal_ok :
    'a Alcotest.testable -> (Rpc.t -> 'a) -> 'a -> Rpc.t -> unit =
...
let test_char () =
  check_marshal_unmarshal
    ( 'x'
    , Rpc.Int (Char.code 'x' |> Int64.of_int)
    , rpc_of_test_char
    , test_char_of_rpc )
...
type test_variant = VNone | VOne of int | VTwo of (int * int)
[@@deriving rpc]
...
type test_variant_name = VThree of int [@name "bob"] | VFour [@name "lofty"]
[@@deriving rpc]
...
type test_record = {fiEld1: int; fiEld2: string} [@@deriving rpc]
...
type test_record_opt = {field3: int option; field4: string option}
[@@deriving rpc]
...
let test_default_enum () =
  check_unmarshal_ok
    ...
    enum_of_rpc `default (Rpc.String "unknown_enum") ;
  check_unmarshal_ok
    (Testable.from_rpc_of_t rpc_of_enum)
    enum_of_rpc `default
    (Rpc.Enum [Rpc.String "thRee"; Rpc.Enum [Rpc.Int 4L; Rpc.Int 5L]]) ;
...
let test_enum_string_map () =
  check_marshal_unmarshal
    ( [(`x, "x"); (`y, "y"); (`z, "z")]
    , Rpc.Dict
        [("x", Rpc.String "x"); ("y", Rpc.String "y"); ("z", Rpc.String "z")]
...
let test_enum_string_map2 () =
  check_marshal_unmarshal
    ( [(`a, "x"); (`b, "y"); (`c, "z")]
    , Rpc.Dict
        [("a", Rpc.String "x"); ("b", Rpc.String "y"); ("c", Rpc.String "z")]
