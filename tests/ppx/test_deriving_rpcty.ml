...
let test_string () =
  check_marshal_unmarshal ("test string", Rpc.String "test string", typ_of_test_string)
...
let test_char () =
  check_marshal_unmarshal ('x', Rpc.Int (Char.code 'x' |> Int64.of_int), typ_of_test_char)
...
let test_tuple2 () =
  check_marshal_unmarshal
    ((3, "hello"), Rpc.Enum [ Rpc.Int 3L; Rpc.String "hello" ], typ_of_test_tuple2)
...
let test_option_none () = check_marshal_unmarshal (None, Rpc.Enum [], typ_of_test_option)
...
let test_variant_name () =
  check_marshal_unmarshal
    (VThree 5, Rpc.Enum [ Rpc.String "bob"; Rpc.Int 5L ], typ_of_test_variant_name)
...
let test_record () =
  check_marshal_unmarshal
    ...
    , Rpc.Dict
        [ "fiEld1", Rpc.Int 7L; "fiEld2", Rpc.String "banana"; "fiEld3", Rpc.Bool false ]
    , typ_of_test_record )
...
let test_record_one_field () =
  check_marshal_unmarshal
    ({ field = true }, Rpc.Dict [ "field", Rpc.Bool true ], typ_of_test_record_one_field)
...
let fakegen () =
  let fake ty =
    ...
    let test2 =
      List.map (fun json -> Rpcmarshal.unmarshal ty (Jsonrpc.of_string json)) ss
    in
