...
type t = Foo of int | Bar of (int * float) [@@deriving rpc]
...
let run () =
  ...
  Alcotest.(check Testable.response)
    "Sanity check success=s_json" success s_json ;
  Alcotest.(check Testable.response)
    "Sanity check failure=f_json" failure f_json
