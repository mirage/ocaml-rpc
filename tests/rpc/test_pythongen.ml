module Interface (R : Idl.RPC) = struct
  ...
  let int_p_result =
    Idl.Param.mk ~name:"int" ~description:["int result"] Rpc.Types.int

  let add =
    R.declare "add" ["Add two numbers"]
      ( int_p_named_1 @-> int_p_named_2
      @-> returning int_p_result Idl.DefaultError.err )

  let bool_p_named_1 =
    Idl.Param.mk ~name:"bool1" ~description:["first bool param"] Rpc.Types.bool

  let bool_p_named_2 =
    Idl.Param.mk ~name:"bool2" ~description:["second bool param"]
      Rpc.Types.bool

  let bool_p_result = Idl.Param.mk ~description:["bool result"] Rpc.Types.bool

  let _land =
    R.declare "land" ["Logical and"]
      (bool_p_named_1 @-> bool_p_named_2 @-> returning bool_p_result errors)

  let unit_p = Idl.Param.mk Rpc.Types.unit

  let noop =
    R.declare "noop" ["Do nothing"] (bool_p_named_1 @-> returning unit_p errors)
...
let interfaces =
  Codegen.Interfaces.create ~name:"test_interface" ~title:"Test Interface"
...
let run ?input cmd =
  ...
  (match input with Some input -> output_string out input | None -> ()) ;
  close_out out ;
...
let test_commandline () =
  ...
  Alcotest.(check string)
    "Calc.add with parameters passed on the command line" "9" n ;
  let n = run ~input:{|{"int1":3,"int2":2}|} "Calc.add --json" in
  Alcotest.(check string)
    "Calc.add with parameters passed to stdin as JSON" "5" n ;
  let b = run "Calc.land false true" in
  Alcotest.(check string)
    "Calc.land with parameters passed on the command line" "false" b ;
  let b = run ~input:{|{"bool1":true,"bool2":true}|} "Calc.land --json" in
  Alcotest.(check string)
    "Calc.land with parameters passed to stdin as JSON" "true" b ;
  let b = run "Calc.noop false" in
  Alcotest.(check string)
    "Calc.noop with parameters passed on the command line" "null" b ;
