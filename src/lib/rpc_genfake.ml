...
let rec gentest : type a. Seen.t -> a typ -> a Seq.t =
  ...
  then Seq.empty
    ...
    | Tuple4 (t1, t2, t3, t4) ->
      ...
      Seq.(
        product (product v1s v2s) (product v3s v4s)
        |> map (fun ((x, y), (z, t)) -> x, y, z, t))
    ...
    | Variant { variants; _ } ->
      variants
      |> List.to_seq
      |> Seq.map (function Rpc.Types.BoxedTag v ->
...
let rec genall : type a. maxcomb:int -> Seen.t -> int -> string -> a typ -> a Seq.t =
  ...
  let thin d result = if d < 0 then Seq.take 1 result else Seq.take maxcomb result in
  let seen_t = SeenType.T t in
  if Seen.mem seen_t seen
  then Seq.empty
  else (
    ...
    | Array typ ->
      thin depth ([ genall (depth - 1) strhint typ |> Array.of_seq; [||] ] |> List.to_seq)
    | List typ ->
      thin depth ([ genall (depth - 1) strhint typ |> List.of_seq; [] ] |> List.to_seq)
    | Dict (basic, typ) ->
      ...
      let all_combinations =
        Seq.fold_left
          (fun acc (f, max) ->
             Seq.ints 1
             |> Seq.take max
             |> Seq.flat_map @@ fun i -> Seq.map (fun dict -> (f, i - 1) :: dict) acc)
          (Seq.return [])
          fields_maxes
      in
      Seq.map
    ...
    | Variant { variants; _ } ->
      variants
      |> List.to_seq
      |> Seq.flat_map (function Rpc.Types.BoxedTag v ->
...
let genall ?(maxcomb = Sys.max_array_length) depth strhint t =
  genall ~maxcomb Seen.empty depth strhint t |> List.of_seq
