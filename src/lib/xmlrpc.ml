    ...
    if not (Xmlm.eoi input) then
      ...
      | `El_end -> (
        ...
        | h :: t ->
            ...
            Buffer.add_string buf ">" ;
            aux t )
      | `Data d -> Buffer.add_string buf d ; aux tags
      | `Dtd _ -> aux tags
  in
...
module Parser = struct
  ...
  let get_data input =
    ...
    | `Data d -> d
    | `El_start ((_, tag), _) ->
        parse_error (sprintf "open_tag(%s)" tag) "data" input
    | `El_end -> ""
  ...
  let rec open_tag input =
    ...
    | `Data d ->
        parse_error (sprintf "data(%s)" (String.escaped d)) "open_tag" input
  ...
  let rec close_tag tag input =
    ...
    | `El_end -> ()
    | `El_start ((_, t), _) ->
        parse_error (sprintf "open_tag(%s)" t)
  ...
  let map_tags f input =
    ...
    let r =
      if Xmlm.peek input = `El_end then empty_tag input tag else f input tag
    in
    close_tag tag input ; r
    ...
    if t = "value" then (
      let r =
        ...
        | `Data d ->
            ...
            if
              is_empty d
              && match Xmlm.peek input with `El_start _ -> true | _ -> false
            then f input
            else Rpc.String d
        | _ -> f input
      in
      close_tag "value" input ; r )
...
let of_string ?callback str =
  ...
  (match Xmlm.peek input with `Dtd _ -> ignore (Xmlm.input input) | _ -> ()) ;
...
let call_of_string ?callback str =
  ...
  let name = ref "" in
  let params = ref [] in
  Parser.map_tag "methodCall"
    (fun input ->
      name := Parser.map_tag "methodName" Parser.get_data input ;
      Parser.map_tag "params"
        (fun input ->
          Parser.skip_empty input ;
          while Xmlm.peek input <> `El_end do
            Parser.map_tag "param"
              (fun input ->
                params := Parser.of_xml ?callback [] input :: !params )
              input ;
            Parser.skip_empty input
          done )
        input )
...
let response_of_input ?callback input =
  ...
  Parser.map_tag "methodResponse"
    (fun input ->
      ...
      | `Data d ->
          parse_error (String.escaped d) "open_tag(fault/params)" input
