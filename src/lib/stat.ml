...
module StatTree = struct
  ...
  let rec get_stat : string list -> 'a t -> Stat.t = fun path t ->
    match path, t with
    | [], Node (st,_) -> st
    | s::ss, Node (_st,children) -> get_stat ss (List.assoc s children)

  let rec update_stat : int64 -> string list -> 'a t option -> 'a t = fun g path t ->
    match path, t with
    | [], Some (Node (st, xs)) -> Node (Stat.update g st, xs)
    | [], None -> Node (Stat.make g, [])
    | s::ss, None ->
      Node (Stat.make g, [s, update_stat g ss None])
    | s::ss, Some (Node (st, xs)) ->
      ...
      let new_mine =
        match mine with
        | None -> (s, update_stat g ss None)
        | Some (_, n) -> (s, update_stat g ss (Some n))
      in
...
let map_fld : type b. (b, 'a) Rpc.Types.field -> (int64, 'a wrapped_field StatTree.t) Rpc.Types.field = fun f ->
  ...
  { fname = f.fname;
    ...
    fset = (fun g st -> StatTree.update_stat g path (Some st));
    fcls = Stats;
    fdescription = [];
    fversion = None;
    fdefault = None;
