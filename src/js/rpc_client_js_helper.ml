...
let is_array obj =
  let str =
    Js_of_ocaml.Js.Unsafe.call (Js_of_ocaml.Js.Unsafe.variable "Object.prototype.toString") obj [||]
  in
...
let rec rpc_of_json json =
  ...
  | "object" ->
      ...
      else if is_string json then
        Rpc.String (Js_of_ocaml.Js.to_string (Js_of_ocaml.Js.Unsafe.coerce json))
      else if is_null json then Rpc.Null
      else
        let okeys = keys json in
        Rpc.Dict
          (List.map
             (fun x -> (x, rpc_of_json (Js_of_ocaml.Js.Unsafe.get json (Js_of_ocaml.Js.string x))))
             okeys)
...
let to_string rpc =
  let rec inner = function
    | Rpc.Dict kvs ->
        ...
        List.iter (fun (x, y) -> Js_of_ocaml.Js.Unsafe.set o (Js_of_ocaml.Js.string x) (inner y)) kvs ;
        o
...
let get name dict =
  if List.mem_assoc name dict then List.assoc name dict
  else (
