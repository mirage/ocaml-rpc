...
let is_array obj =
  let str =
    Js.Unsafe.call (Js.Unsafe.variable "Object.prototype.toString") obj [||]
  in
...
let is_string obj = Js.instanceof obj mlString_constr
...
let rec rpc_of_json json =
  ...
  | "object" ->
      if is_array json then
        ...
        Rpc.Dict
          (List.map
             (fun x -> (x, rpc_of_json (Js.Unsafe.get json (Js.string x))))
             okeys)
  | "boolean" -> Rpc.Bool (Js.to_bool (Obj.magic json))
  | "number" ->
  ...
  | _ ->
      (* Datetime maybe? *)
      Firebug.console ## log
        (Js.string (Printf.sprintf "Ack! got %s" (Js.to_string ty))) ;
      Rpc.Bool false
