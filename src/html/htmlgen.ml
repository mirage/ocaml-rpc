  ...
  let row_of_arg (is_in, Param.Boxed arg) =
    ...
    tag "tr"
      (list
         [ tag "td"
             (tag "code"
                (string (match name with Some s -> s | None -> "unnamed")))
...
let sidebar x =
  ...
  let of_interface i =
    ...
    @ List.map
        (of_method i.Interface.details.Idl.Interface.name)
        i.Interface.methods
  in
  div ~cls:"large-3 medium-3 columns"
    (tag "ul" ~cls:"menu vertical"
       (list
...
let of_type_decl _ (BoxedDef t) =
  ...
  let rest =
    match t.ty with
    | Struct structure ->
        [p (string "Members:"); of_struct_fields structure.fields]
    | Variant variant ->
        [p (string "Constructors:"); of_variant_tags variant.variants]
    | _ -> []
  in
  common @ rest
...
let tabs_of _ i m =
  ...
  [ ul ~cls:"tabs" ~id:id_tab ~attrs:[("data-tabs", "")]
      [ li ~cls:"tabs-title is-active"
          (tag "a" ~attrs:[("href", hash_defn); ("aria-selected", "true")]
             (string "Definition"))
      ; li ~cls:"tabs-title"
          (a ~href:(Uri.of_string hash_ocaml) (string "OCaml example"))
      ; li ~cls:"tabs-title"
...
let of_method is i (Codegen.BoxedFunction m) =
  let anchor =
    Printf.sprintf "a-%s-%s" i.Interface.details.Idl.Interface.name
      m.Method.name
  in
  let name = m.Method.name in
    ...
    (list
       ...
       ; div ~cls:"large-9 medium-9 columns"
           (list
              ...
              @ List.concat
                  (List.map (of_type_decl None) x.Interfaces.type_decls)
