...
(* Common parameters *)
let dbg =
  Param.mk ~name:"dbg" ~description:[ "Debug context from the caller" ] Types.string
...
let task_id =
  Param.mk
    ~name:"task_id"
    ~description:
      [ "Opaque string representing the task associated with the"; "operation." ]
    Types.string
...
let domain =
  Param.mk ~description:[ "An opaque string which represents the Xen domain." ] domain
